#!/usr/bin/env python3
"""
Train a temporal transformer on MPOSE2021 sequences.
Uses the JointFeatureExtractor to compute per‑frame joint embeddings,
then aggregates them into a frame‑level vector (mean over joints),
and finally processes the sequence with a transformer classifier.
python temporal_transformer_mpose.py --seq_len 20 --batch_size 64 --epochs 50 --split 1

"""

#!/usr/bin/env python3
"""
Train a temporal transformer on MPOSE2021 sequences using pre‑computed velocities.
"""
...
# Add project root for import of extractor
...
class MPOSESequenceDataset(Dataset):
    """Loads sequences of keypoints from MPOSE2021 (raw format)."""
    def __init__(self, X, y, seq_len=20):
        """
        X: list of numpy arrays, each (T, J, C) with C = 5 (x,y,conf,dx,dy).
        y: list of integer labels.
        seq_len: fixed length to pad/truncate to.
        """
    ...
    def __getitem__(self, idx):
        # return (seq, label) as torch tensors
...
class TemporalTransformerClassifier(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, num_classes, seq_len,
        ...
        self.pos_encoder = PositionalEncoding(hidden_dim, max_len=seq_len+1)   # +1 for CLS token
        ...
        # Classifier with same structure as GraphSAGE (BatchNorm, ReLU, Dropout)
...
def main():
    ...
    parser.add_argument('--freeze_extractor', action='store_true', help='Freeze extractor')
    parser.add_argument('--finetune_extractor_head', action='store_true', help='Only train head of extractor')
    parser.add_argument('--patience', type=int, default=30)
    parser.add_argument('--output_dir', default='./temporal_transformer_mpose')
    ...
    exp_name = f"temporal_transformer_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    # -------------------- Load MPOSE2021 --------------------
    print("Loading MPOSE2021 dataset with velocities...")
    data_dir='./data/mpose'
    ...
    # X_train is list of numpy arrays (T, J, C) with C = 5 (x,y,conf,dx,dy)
    ...
    # -------------------- Model --------------------
    # Build extractor (with velocities=True)
    extractor = MPOSEFeatureExtractor(
        num_joints=17,
        joint_type_embedding_dim=16,
        max_angles=3,
        use_angles=True,
        use_velocities=True,          # uses pre‑computed velocities from dataset
    ...
    if args.freeze_extractor:
        ...
        print("Extractor frozen.")
    elif args.finetune_extractor_head:
        unfrozen = 0
        for name, param in extractor.named_parameters():
            if any(x in name for x in ['fc', 'head', 'output', 'classifier', 'proj']):
                param.requires_grad = True
                unfrozen += 1
            else:
                param.requires_grad = False
        print(f"Fine‑tuning extractor head: {unfrozen} parameter groups.")
    else:
        for param in extractor.parameters():
            param.requires_grad = True
        print("Full fine‑tuning of extractor.")
    extractor.to(device)

    # Temporal transformer
    transformer = TemporalTransformerClassifier(
        input_dim=64,          # after pooling over joints
    ...
    optimizer = torch.optim.AdamW(
        list(extractor.parameters()) + list(transformer.parameters()),
        lr=args.lr,
        weight_decay=1e-4
    )
    ...
    for epoch in range(args.epochs):
        ...
        for seq, labels in loop:
            seq = seq.to(device)          # (B, T, J, C) with C = 5
            labels = labels.to(device)
            B, T, J, C = seq.shape

            # Pass full sequence to extractor (which expects 4D input)
            out = extractor(seq, apply_augmentations=True)
            joint_emb = out['joint_embeddings']   # (B, T, J, 64)
            # Pool over joints -> (B, T, 64)
            frame_vec = joint_emb.mean(dim=2)     # (B, T, 64)

            logits = transformer(frame_vec)
        ...
        with torch.no_grad():
            for seq, labels in val_loader:
                ...
                B, T, J, C = seq.shape
                out = extractor(seq)
                joint_emb = out['joint_embeddings']
                frame_vec = joint_emb.mean(dim=2)
                logits = transformer(frame_vec)
                loss = criterion(logits, labels)
    ...
    extractor.load_state_dict(best_model_state['extractor'])
    transformer.load_state_dict(best_model_state['transformer'])
    extractor.eval()
    transformer.eval()
    all_preds = []
    all_labels = []
